    ...
    $("#btnSendIR").on("click", function () {
        ...
        if (msg == "" || msg == null) {
            ...
            if (confirm("Are You sure you want to Send this Request?")) {
                revealModal('loading');
                request_reprint(reprintTranRow.HEAD_ID, msg, IR_HEAD.TRAN_ID);
    ...
    $("#tblReprintTransactions tbody").on('click', 'tr', function () {
        ...
        if (reprintTranRow != undefined) {
            if (reprintTranRow.STATUS == null || reprintTranRow.STATUS == 'null') {
                ...
                revealModal('modalIRdetails');
                ir.getData(reprintTranRow.TRAN_ID);
                $('#txtItem').val('');
                $('#txtQty').val('');
            }
        }
...
function request_reprint(headId, msg, tranId) {
    //msg = encodeURI(msg);
    var xmlhttp;
    ...
    xmlhttp.send("{'headId':'" + headId + "','message':'" + msg + "', 'tranId': '"+ tranId +"'}");
}
